...
(defun run-model-and-save-output (n)
  ...
  (dotimes (i n)
    ...
    (sgp-fct (list :v (format nil "c:/Users/sfw5621/Desktop/model-output/model-output-~a.txt" i)))
    (run 10)
    (sgp :v nil) ;; make sure the output stream is closed
    ))
  
;; parameters
(sgp :v t :esc t :trace-detail high  :ul t :ult t :egs 4 :MODEL-WARNINGS NIL)
...
;prodcution rules
;start the production of choose strategy
(p choose-strategy
   =goal>
   isa decision
...
;the end of one decision making and dynamic compute reward
(p stop
  =goal>
  isa decision
   state result
...
;set up the start utility for each set rules
 (spp decide-brute :u 3)
 (spp expert-strategy :u 0)
 (spp intermediate-strategy :u 0)
